<template>
  <div class="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
    ...
    <header class="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-30">
      ...
      <button @click="exportFullPreview" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-semibold transition-all shadow-sm shadow-indigo-100 flex items-center gap-2">
    ...
    <!-- MAIN GRID -->
    <main class="max-w-6xl mx-auto p-6 sm:p-12 grid sm:grid-cols-12 gap-12">
      
      <!-- QR PREVIEW: Mobile first -->
      <div class="sm:col-span-5 order-1 sm:order-2 relative">
        <div class="sm:sticky sm:top-28 h-auto sm:h-[calc(100vh-12rem)] w-full flex flex-col items-center justify-center">
      ...
      <!-- CONTROLS: Mobile second -->
      <div class="sm:col-span-7 order-2 sm:order-1 space-y-6">
          ...
          <div class="grid md:grid-cols-2 gap-6">
            <div class="space-y-4">
              ...
              <div class="flex items-center gap-3">
                <input type="color" v-model="qrColor" class="w-10 h-10 rounded-sm cursor-pointer border-none bg-transparent" />
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center gap-4">
                <input type="color" v-model="bgColor" :disabled="isTransparent" class="w-10 h-10 rounded-sm cursor-pointer border-none bg-transparent disabled:opacity-20" />
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <div class="flex gap-2">
                <button v-for="type in ['square', 'dots', 'rounded', 'extra-rounded']" :key="type" 
                  ...
                  class="flex-1 py-2 text-xs font-bold rounded-sm capitalize transition-all">
                  {{ type }}
                </button>
              </div>
            </div>
          ...
          <div v-if="hasFrame" class="space-y-6 animate-in">
            ...
            <div class="grid md:grid-cols-2 gap-6 pt-2">
              <div class="space-y-2">
                ...
                <input v-model="frameText" type="text" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-sm text-sm focus:ring-2 focus:ring-indigo-500 uppercase font-bold" />
              </div>
              <div class="space-y-2">
                <label class="text-sm font-medium text-slate-700">Frame Color</label>
                <input type="color" v-model="frameColor" class="w-full h-10 rounded-sm cursor-pointer border-none bg-transparent" />
              </div>
            </div>
          </div>
        </section>
...
const exportFullPreview = async () => {
  ...
  try {
    const dataUrl = await htmlToImage.toPng(exportArea.value, {
      quality: 1,
      pixelRatio: 3,
      backgroundColor: isTransparent.value ? null : '#f8fafc'
    })
...
onMounted(() => {
  const size = window.innerWidth < 768 ? 150 : 250; // <768px -> mobile, >=768px -> desktop
  ...
  if (!getlink) router.push('/')
  else qrText.value = getlink

  window.addEventListener("resize", () => {
    const newSize = window.innerWidth < 768 ? 150 : 250;
    qrCode.value.update({ width: newSize, height: newSize });
  });
})
</script>
